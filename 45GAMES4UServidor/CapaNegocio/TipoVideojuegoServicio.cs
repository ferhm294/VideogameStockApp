...
{
    public class TipoVideojuegoServicio
    {
        ...
        //Este método va a validar el Id que el usuario quiere utilizar, devuelve true si está disponible
        {
            ...
            foreach (TipoVideojuegoEntidad TipoVideojuego in tipoVideojuegoAD.ConsultaTipoVideojuego())
            {
                if (TipoVideojuego == null)
                {
                    return true;
                }
                else if (TipoVideojuego.Id == Id)
                {
                    return false;
                }
            }
